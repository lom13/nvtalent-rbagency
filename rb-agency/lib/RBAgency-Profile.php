	 ...
	 */
		public static function search_result_public($sql, $castingcart = '',$shortcode = false,$arr_query = array()){
			...
			if ($count > 0){
				...
				$profiles_per_col = 0;
				$profiles_per_col = $count / 4;
				$profiles_per_col = round($profiles_per_col);

				// $all_html.= "<h1>".$count."</h1>";
				...
				foreach($results as $profile) {
					...
					if($rb_agency_option_layoutprofileviewmode == 2 && $rb_agency_option_layoutprofilelistlayout !=1){
						...
						$profilesPerRow++;
					} elseif ($arr_query['list_layout'] == 'voiceover') {
						// if(($profilesPerRow % $profiles_per_col == 1) || $profiles_per_col == 1) {
						// 	$profile_list .= "<div class='vertical-col'>";
						// }
						$profile_list .= self::search_formatted($profile, $arr_favorites, $arr_castingcart, $availability, false, $arr_query );
						// if($profilesPerRow % $profiles_per_col == 0) {
						// 	$profile_list .= "</div>";
						// }
						// $profilesPerRow++;
					} else {						
			 ...
			 */
			 ...
			 $queryMedia = "SELECT * FROM " . table_agency_profile_media . " WHERE ProfileID =  '%d' AND ProfileMediaType 
			        ...
			        // WHERE ProfileID = '%d' AND ProfileMediaType='rbcustommedia_commercial_button_mp3_1'";
							...
							// Attributes | ProfileMediaType
							$mediaTypes = explode(",", $arr_query['profileumltitype']);
							...
							*/
							...
							// Merge all data of user in one array to create separation of categories per user.
							$categories = array(
								1  => array(),
							  	2 => array(),
							  	3  => array(),
							...
							// Filter which should show according to ProfileType on shortcode
							for ($i=0; $i < count($mediaTypes); $i++) {
								foreach ($resultsMedia as $key => $result) {
									// Get Profiletype ID of the data from database
									$profileTypeMedia = explode('_', $result['ProfileMediaType'])[4];
									// Get only those ProfileTypes what's inside the shortcode. If admin did not include the id it shouldn't be here
									if ( $mediaTypes[$i] == $profileTypeMedia ) {
										// Use the var Categories to merge them
										array_push($categories[$profileTypeMedia], $result);
									}
